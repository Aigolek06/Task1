    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < a; i++)// create a cycle that runs through numbers of array
            {
                ...
                for (int j = 2; j <= Math.Sqrt(b); j++) //create a cycle that run from 2 to mean sqrt of integer in array
                {
                    if (b % j == 0) // to find all divisors of b ny this condition
                        count++;  //
            ...
            Console.ReadKey();   //do not close cmd
        }
    }
}
